...
def modify_grad_expl_mask(x, inds, factor=-1.):
    inds = inds.expand_as(x)
    # print(inds)
    x[inds] *= factor
    return x
...
def modify_grad_reverse(x, factor=-0.2):
    x = x * factor
    return x
...
def evaluation_batch_RD(encoder, bn, decoder, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    encoder.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
...
def evaluate_sp(encoder, bn, decoder, dataloader, device, _class_=None):
    ...
    pr_list_sp = []
    with torch.no_grad():
        tqdm_obj = tqdm(range(len(dataloader)))
        for iteration, (img, gt, label, _) in zip(tqdm_obj, dataloader):
...
def global_cosine_hm_exclude(a, b, p=0.5, factor=0.):
    if a[0].shape[0] == 0:
        return 0
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0
...
def global_cosine_hm_circle(a, b, p=0.9, p_f=0.99, factor=0.):
    if a[0].shape[0] == 0:
        return 0
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0
    for item in range(len(a)):
        ...
        # mean_dist = point_dist.mean()
        # std_dist = point_dist.reshape(-1).std()
        thresh1 = torch.topk(point_dist.reshape(-1), k=int(point_dist.numel() * (1 - p)))[0][-1]
        thresh2 = torch.topk(point_dist.reshape(-1), k=int(point_dist.numel() * (1 - p_f)))[0][-1]

        loss += torch.mean(1 - cos_loss(a_.reshape(a_.shape[0], -1),
                                        b_.reshape(b_.shape[0], -1)))
        inds1 = point_dist < thresh1
        inds2 = point_dist > thresh2

        partial_func = partial(modify_grad, inds=inds1 + inds2, factor=factor)
        b_.register_hook(partial_func)
...
def global_cosine_expl(a, b, p=0.9, factor=0.):
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0

    # masks = masks.repeat(1, a[0].shape[1], 1, 1)
    for item in range(len(a)):
        a_ = a[item].detach()
        b_ = b[item]
        # with torch.no_grad():
        ...
        partial_func = partial(modify_grad_expl_mask, inds=masks, factor=factor)
        # partial_func = partial(modify_grad_reverse, inds=masks, factor=factor)
        b_.register_hook(partial_func)
    loss = loss / len(a)
    return loss


def global_cosine_hm_percent_w(weights, a, b, p=0.9, factor=0.):
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0
    for item in range(len(a)):
        a_ = a[item].detach()
        ...
        loss += torch.mean((1 - cos_loss(a_.reshape(a_.shape[0], -1),
                                         b_.reshape(b_.shape[0], -1))) * torch.tensor(weights, dtype=a_.dtype,
                                                                                      device=a_.device))
...
def global_cosine_hm_percent_sum(a, b, p=0.9, factor=0.):
    if a[0].shape[0] == 0:
        return 0
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0
    for item in range(len(a)):
        ...
        loss += torch.sum(1 - cos_loss(a_.reshape(a_.shape[0], -1),
                                       b_.reshape(b_.shape[0], -1)))
...
def global_cosine_hm_sample(a, b, p=0.9, factor=0.):
    cos_loss = torch.nn.CosineSimilarity()
    drop = DropBlock2D(block_size=2, drop_prob=0.3)
    loss = 0
    for item in range(len(a)):
        a_ = a[item].detach()
        b_ = b[item]
        ...
        # std_dist = point_dist.reshape(-1).std()
        # thresh = torch.topk(point_dist.reshape(-1), k=int(point_dist.numel() * (1 - p)))[0][-1]
        mask = torch.ones([a_.shape[0], 1, a_.shape[2], a_.shape[3]])
        mask = drop(mask)
        mask = mask != 0
        loss += torch.mean(1 - cos_loss(a_.reshape(a_.shape[0], -1),
                                        b_.reshape(b_.shape[0], -1)))
        ...
        # partial_func = partial(modify_grad, inds=point_dist < thresh, factor=factor)
        partial_func = partial(modify_grad, inds=mask, factor=factor)
        b_.register_hook(partial_func)
...
def generate_random_noise(B, C, H, W, std_dev, variance=None):
    if variance is not None:
        std_dev = np.sqrt(variance)  # Calculate standard deviation
    elif std_dev is None:
        raise ValueError("Either std_dev or variance must be provided")

    # Generate random noise matrix
    noise = np.random.normal(loc=0.0, scale=std_dev, size=(B, C, H, W))
    return noise


def global_cosine_hm_masknoise(masks, a, b, p=0.9, factor=0.):
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0
    for item in range(len(a)):
        a_ = a[item].detach()
        b_ = b[item]
        # Add noise to encoder features using masks
        masks = torch.tensor(masks, dtype=a_.dtype, device=a_.device).unsqueeze(1)
        masks = F.interpolate(masks, size=a_.shape[2], mode='bilinear', align_corners=True)
        noise = generate_random_noise(a_.shape[0], a_.shape[1], a_.shape[2], a_.shape[3], torch.std(a_).item())
        a_ = (a_ * ~masks) + (a_ * masks + noise)
        with torch.no_grad():
            point_dist = 1 - cos_loss(a_, b_).unsqueeze(1)
        # mean_dist = point_dist.mean()
        # std_dist = point_dist.reshape(-1).std()
...
def regional_cosine_segmentation(a, b, masks, out_size=28, epoch=0, c=None, imgs=None, t=None):
    # focal_loss = FocalLoss(gamma=0.4)
    ...
    # plt.show()
    a_map_list = []
    masks = torch.tensor(masks, dtype=a[0].dtype, device=a[0].device).unsqueeze(1)
    for i in range(len(a)):
        ...
        # partial_func = partial(modify_grad, inds=masks == 0, factor=0.1)
        # ft.register_hook(partial_func)
    anomaly_map = torch.cat(a_map_list, dim=1).mean(dim=1, keepdim=True)
    # anomaly_map = F.interpolate(anomaly_map, size=256, mode='bilinear', align_corners=False)
    # anomaly_map = gaussian_kernel(anomaly_map).squeeze(1)

    # plt.imshow(masks[0], cmap='gray')
    # plt.show()

    # nonzero_indices = torch.nonzero(masks)
    # print("{}::{}".format(torch.max(anomaly_map), torch.max(masks)))
    # a = torch.max(anomaly_map)
    # b = torch.max(masks)
    # loss = torch.mean(mse_loss(anomaly_map, masks))
    loss = mse_loss(anomaly_map, masks)
    return loss


def global_cosine_reverse(a, b):
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0
    for item in range(len(a)):
        loss += torch.mean(1 + cos_loss(a[item].view(a[item].shape[0], -1),
...
# def global_cosine_hm_reverse(a, b, p=0.9, factor=0.):
#     cos_loss = torch.nn.CosineSimilarity()
#     loss = 0
#     for item in range(len(a)):
#         a_ = a[item].detach()
...
def regional_cosine_segmentationK(a, b, masks, out_size=28, epoch=0, c=None, imgs=None, t=None):
    # focal_loss = FocalLoss(gamma=0.4)
    mse_loss = torch.nn.MSELoss(reduction='none')
    # cos_loss = torch.nn.CosineSimilarity()
    # masks = (masks - np.min(masks)) / (np.max(masks) - np.min(masks))
    ...
    for i in range(len(a)):
        ...
        a_map_list.append(a_map)
    anomaly_map = torch.cat(a_map_list, dim=1).mean(dim=1, keepdim=True)
    # diff = (anomaly_map - torch.tensor(masks, device=anomaly_map.device)) ** 2
    # d_hard = torch.quantile(diff, q=0.99)
    # num = torch.sum(diff >= d_hard)
    # loss = (diff[diff >= d_hard]).mean()
    loss = torch.mean(mse_loss(anomaly_map, masks), dim=(1, 2, 3))
    k = 1 - loss
    k = (k - torch.min(k)) / (torch.max(k) - torch.min(k))
    k[k < 0.5] = 0
    # https://zhuanlan.zhihu.com/p/66281890
    # print(k)
    # w = 1 - F.softmax(loss * t)
    # if epoch >= 2:
    #     for i in range(masks.shape[0]):
    #         print(c[i])
    #         show_image(imgs[i].detach().cpu())
    #         show_image(anomaly_map[i].detach().cpu())
    #         show_image(masks[i].detach().cpu())
    loss = torch.mean(loss * k)
    # loss = torch.mean(1 - cos_loss(anomaly_map.view(1, -1), masks.flatten().view(1, -1)))
    # loss = diff.mean()
    return loss
...
#
# def evaluation_batch_org(args, model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
#     model.eval()
#     gt_list_px = []
#     pr_list_px = []
#     gt_list_sp = []
...
#
#     # return [auroc_sp, ap_sp, f1_sp, auroc_px, ap_px, f1_px, aupro_px]
#     return [auroc_sp, ap_sp, f1_sp, 0., 0., 0., 0.]


def evaluation_batch(args, model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    model.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
    ...
    with torch.no_grad():
        for img, gt, label, img_path in dataloader:
            img = img.to(device)
            # starter.record()
            output = model(img, args, is_test=True)
            # ender.record()
            # torch.cuda.synchronize()
            # curr_time = starter.elapsed_time(ender)
            en, de = output[0], output[1]
...
def evaluation_batch_train(args, model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    model.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
...
def evaluation_batch_uncertainty(args, model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None, n=1):
    model.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
    ...
    with torch.no_grad():
        ...
        for iteration, (img, gt, label, img_path) in zip(tqdm_obj, dataloader):
            scores = []
            anomaly_maps = []
            img = img.to(device)
            # starter.record()
            ...
            # for i in range(n):
            #     output_ = model(img, args, is_test=True)
            #     en, de = output_[0], output_[1]
            #     anomaly_map, _ = cal_anomaly_maps(en, de, img.shape[-1])
            #     if resize_mask is not None:
            #         anomaly_map = F.interpolate(anomaly_map, size=resize_mask, mode='bilinear', align_corners=False)
            #         gt = F.interpolate(gt, size=resize_mask, mode='nearest')
            ...
            if max_ratio == 0:
                ...
                sp_score = sp_score.mean(dim=1)
            pr_list_sp.append(sp_score)
            # pr_list_sp.append(sp_score + stds)
...
def evaluation_batch_org(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    print("test eval")
    model.eval()
    gt_list_px = []
    pr_list_px = []
...
def evaluation_batch_full(args, model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    model.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
