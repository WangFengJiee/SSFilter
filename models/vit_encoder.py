...
def load(name):
    ...
    if "amd" in name:
        amd_path = os.environ.get("AMD_VIT_PATH", "backbones/weights/amd_vitb_IN1K_pretrain_200e_encoder.pth")
        state_dict = torch.load(amd_path, map_location='cpu')['model']
        state_dict = remove_encoder_prefix(state_dict)

    if "clip" in name:
        clip_path = os.environ.get("CLIP_VIT_PATH", "backbones/weights/vit_b_16_plus_240-laion400m_e32-699c4b84.pt")
...
def remove_encoder_prefix(ordered_dict):
