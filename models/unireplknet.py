...
class BN_layer(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(4):
            if i == 3:
                main_stage = nn.Sequential(  # kernel_sizes[i][j]
                    *[UniRepLKNetBlock(dim=dims[i], kernel_size=3, drop_path=dp_rates[cur + j],
                                       layer_scale_init_value=layer_scale_init_value, deploy=deploy,
                                       attempt_use_lk_impl=attempt_use_lk_impl,
                                       with_cp=with_cp, use_sync_bn=use_sync_bn) for j in
                      range(depths[i])])
        ...
        self.bn3 = nn.BatchNorm2d(dims[2])
        self.conv4 = conv3x3(dims[2] * 3, dims[3], 2)
        # self.drop_block = DropBlock2D(block_size=13, drop_prob=0.4)
        self.drop = nn.Dropout(0.4)
        self.norms = nn.ModuleList()
        for i in range(depths[3]):
            self.norms.append(LayerNorm(dims[3], eps=1e-6, data_format="channels_first"))
        self.n1 = LayerNorm(dims[3], eps=1e-6, data_format="channels_first")
        for m in self.modules():
    ...
    #     feature = self.n1(feature)
    #
    #     for idx, stage in enumerate(self.stages[0]):
    #         feature = stage(feature)
    #         feature = self.drop(feature)
    #         feature = self.norms[idx](feature)
    #     return feature
...
def unireplknet_n(in_1k_pretrained=False, **kwargs):
    ...
    bn = BN_layer(depths=UniRepLKNet_N_depths, dims=(80, 160, 320, 640), **kwargs)
...
def unireplknet_t(in_1k_pretrained=False, **kwargs):
    ...
    bn = BN_layer(depths=UniRepLKNet_T_depths, dims=(80, 160, 320, 640), **kwargs)
...
def unireplknet_s(in_1k_pretrained=False, in_22k_pretrained=False, in_22k_to_1k=False, **kwargs):
    ...
    bn = BN_layer(depths=UniRepLKNet_S_B_L_XL_depths, dims=(96, 192, 384, 768), **kwargs)
